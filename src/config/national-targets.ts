...
// Expert Douche Senior - Couverture Nationale pSEO
// ========================================
...
export function getTargetAsCityConfig(slug: string): CityConfig | undefined {
    ...
    const priceDesc = "Éligible MaPrimeAdapt'";

    const title = `Douche Senior ${target.name} | Remplacement Baignoire & PMR`;
    const uniqueDescription = `Expert Installation de Douche Sécurisée pour Senior à ${target.name} (${target.zip}). Remplacement de baignoire par douche italienne de plain-pied. Artisans agréés Handibat.`;
    const uniqueMetaDescription = `Installation Douche Sécurisée ${target.name}. Devis gratuit remplacement baignoire par douche senior de plain-pied. Éligible aides MaPrimeAdapt' (jusqu'à 70% d'aides).`;

    return {
        slug: target.slug,
        city: target.name,
        name: `Douche Senior ${target.name}`,
        domain: `expertdouchesenior.com/ville/${target.slug}`,
