...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
    ...
    if (!post) {
        return { title: 'Article non trouvé | Expert Douche Senior' };
    }

    return {
        title: post.seo_title || `${post.title} | Expert Douche Senior`,
        ...
        openGraph: {
            ...
            authors: [post.author_name || 'Expert Douche Senior'],
        },
    };
}
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const schema = {
        ...
        "author": {
            "@type": "Person",
            "name": post.author_name || "Expert Douche Senior"
        },
        "publisher": {
        ...
        "mainEntityOfPage": {
            ...
            "@id": "https://www.expertdouchesenior.com/blog/"
    ...
    return (
        <main className="min-h-screen bg-white text-neutral-900 font-sans selection:bg-teal-100 selection:text-teal-900 pt-20">
             <script
                    ...
                    <aside className="lg:col-span-4 space-y-8">
                        ...
                        <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-teal-100 flex items-center justify-center text-teal-600 font-bold overflow-hidden relative">
                                    {(post.author_name || 'E')[0]}
                                      {/* Verified Badge */}
                                    <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 shadow-sm">
                                        <div className="bg-teal-500 rounded-full w-3 h-3 border-2 border-white"></div>
                                    </div>
                                </div>
                                <div>
                                    <Link href={post.author_slug ? `/author/${post.author_slug}` : "#"} className="font-bold text-neutral-900 hover:text-amber-600 transition-colors">{post.author_name || 'Expert Douche Senior'}</Link>
                                    <p className="text-xs text-neutral-500">Expert Accessibilité & PMR</p>
                                </div>
                            </div>
                        </div>
                         ...
                         <div className="bg-gradient-to-br from-blue-900 to-blue-700 text-white rounded-2xl p-8 text-center shadow-lg">
                            ...
                            <p className="text-blue-100 text-sm mb-6">Comparez les prix des experts en douche senior près de chez vous.</p>
