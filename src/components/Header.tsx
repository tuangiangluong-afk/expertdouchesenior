...
export default function Header({
    ...
    const btnClass = buttonColors[themeColor] || buttonColors.teal;
                ...
                <div className="flex items-center gap-4">
                    ...
                    {isHub && (
                        <div className={`hidden md:flex items-center gap-6 text-sm font-medium ${variant === "light" ? "text-slate-300" : "text-slate-600"}`}>
                            <Link href="/guides/ma-prime-adapt-mode-d-emploi" className="hover:text-teal-500 transition">MaPrimeAdapt&apos;</Link>
                            <Link href="/guides/prix-remplacement-baignoire-douche-senior" className="hover:text-teal-500 transition">Tarifs</Link>
                            <Link href="/guides" className="hover:text-teal-500 transition">Guides</Link>
                        </div>
                    )}

                    {/* Handibat Badge (Desktop) */}
                    <div className="hidden lg:flex items-center gap-2 bg-teal-500/10 border border-teal-500/20 px-3 py-1.5 rounded-full">
                        <span className="w-2 h-2 rounded-full bg-teal-500 animate-pulse"></span>
                        <span className="text-xs font-bold text-teal-600">Agréé Handibat / PMR</span>
                    </div>
